...
def cpp_run(
    ...
    if not interactive:
        ...
        subprocess.run([exec_path.resolve()], check=True)
...
def java_run(
    ...
    if not interactive:
        ...
        subprocess.run(["java", exec_path], check=True)
...
def compile_source_code(
    ...
    if not gen_compile:
        click.secho("Compiling the source code with command:", fg="cyan")
    else:
        click.secho("Compiling the generator code with command:", fg="cyan")
    ...
    if compile_returncode != 0:
        click.secho("Compilation Error:\n", fg="red")
        print_file(compilation_error_path, 2)
        sys.exit()
    else:
        click.secho("Compiled Successfully\n", fg="green")
        if not is_file_empty(compilation_error_path):
            click.secho("Compilation Warning:\n", fg="cyan")
...
def judge(
    ...
    if not interactive:
        ...
        if tc_list == []:
            click.secho("No testcase to run!", fg="red")
            sys.exit(1)
        ...
        for num in tc_list:
            ...
            if not in_path.is_file() and not ans_path.is_file():
                ...
                click.secho(
                    "Input file and Answer file both are not present\n",
                    fg="red",
                    err=True,
                )
                continue
                ...
                click.secho(f"Skipped #{num}\n", fg="yellow")
                click.secho("Input file is not present\n", fg="red", err=True)
                continue
            elif not ans_path.is_file():
                click.secho(f"Skipped #{num}\n", fg="yellow")
                click.secho("Answer file is not present\n", fg="red", err=True)
                continue
            ...
            if check_diff(std_output_path, ans_path):
                ...
                click.secho("Correct Answer: ", fg="cyan")
                print_file(ans_path)
