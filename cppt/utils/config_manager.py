...
def check_config_data(data):
    ...
    if "default_base_folder" in data:
        ...
        default_base_folder_path = default_base_folder_path.expanduser()
        ...
        click.secho(
            "The default base folder was not found in config", err=True, fg="red"
        )
        is_invalid = True
    ...
    if "default_language" in data:
        if data["default_language"] not in ["cpp", "py", "java"]:
            click.secho(
                f"Invalid default language {data['default_language']}",
                err=True,
                fg="red",
            )
            click.secho(
                "python(py), java, c++(cpp) are only supported", err=True, fg="red"
            )
            is_invalid = True
    else:
        click.secho(
            "The default language was not defined in config", err=True, fg="red"
        )
        is_invalid = True
...
def check_config():
    if not user_config_path.exists():
        click.secho(
            f"config not found at {user_config_path.resolve()}", err=True, fg="red"
        )
        click.secho(f"creating config at {user_config_path.resolve()}", fg="cyan")
        create_config()


def get_config_data():
    check_config()
    user_config_file = open(user_config_path, "r", encoding="utf-8")
