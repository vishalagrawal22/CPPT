...
class Task:
    ...
    def create_root(self):
        try:
            ...
            click.secho("Created cppt root folder",
                        fg="cyan")
        except Exception:
    ...
    def create_task(self, tests=None):
        ...
        try:
            ...
            click.secho("Created source code file",
                        fg="cyan")
        except Exception:
        ...
        try:
            ...
            click.secho("Created task folder",
                        fg="cyan")
        except Exception:
            click.secho(f"Unable to create task folder",
                        fg="red", err=True)
            sys.exit(1)

        try:
            self.tc_folder.mkdir()
            click.secho("Created tc folder",
                        fg="cyan")
        except Exception:
            click.secho(f"Unable to create tc folder",
                        fg="red", err=True)
            sys.exit(1)

        try:
            self.last_run_folder.mkdir()
            click.secho("Created last run folder",
                        fg="cyan")
        except Exception:
        ...
        try:
            ...
            for individual_testcase in tests:
                ...
                for testcase_element in individual_testcase:
                    ...
                    with open(self.tc_folder / f"{prefix}{testcase_number}.txt", "w") as file:
                        file.writelines(individual_testcase[testcase_element])
                    is_input_file = is_input_file ^ 1

                testcase_number += 1
        except:
            click.secho(f"Unable to add testcases",
                        fg="red", err=True)
            sys.exit(1)
    ...
    def overwrite(self):
        if self.task_folder.exists():
            try:
                ...
                click.secho("Deleted the existing task folder",
                            fg="cyan")
            except:
                click.secho(f"Unable to delete task folder at {self.task_folder}",
                            fg="red", err=True)
                sys.exit(1)
        ...
        if self.source_code.exists():
            try:
                ...
                click.secho("Deleted the existing source code file",
                            fg="cyan")
            except:
                click.secho(f"Unable to delete source code at {self.source_code}",
                            fg="red", err=True)
                sys.exit(1)
